...
A hand-constructed store is passed as `store` in `config/resilience.ts` — it takes precedence over `default` / `stores`:
...
Then resolve it wherever you build a breaker. The `ResilienceService` exposes **`circuitStore(name?)`**: with no argument it returns the configured default store; with a name it returns the store registered under that key in `stores` (and throws for an unknown name). That is the seam for pointing an explicit `circuitBreaker` at a specific store:

```ts
import resilience from '@adonis-agora/resilience/services/main'
import { circuitBreaker } from '@adonis-agora/resilience'

// The default store (your custom one, when configured as `store`):
circuitBreaker({ key: 'payments', store: resilience.circuitStore(), threshold: 5, cooldownMs: 30_000 })

// A specific named store from config.stores:
