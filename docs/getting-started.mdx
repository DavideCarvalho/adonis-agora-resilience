...
description: Install @adonis-agora/resilience, wrap a flaky call with a composed policy, then register named policies in config and reach them through the container.
icon: Rocket
---
...
This registers `@adonis-agora/resilience/resilience_provider` in `adonisrc.ts` and writes `config/resilience.ts`. `@adonis-agora/diagnostics` and `@adonis-agora/context` are **optional** — install them only if you want diagnostics emission or tenant-aware breaker keys (see [Integrations](/docs/resilience/integrations)).
...
The provider binds a singleton `ResilienceService` built lazily from this config. Import the ready-to-use singleton from `@adonis-agora/resilience/services/main` and run a named policy:
